...
beforeAll(() => {
  // Assign with correct type (no any)
...
describe("edge redirect", () => {
  it("301s via fallback and warms cache", async () => {
    const req = new Request("https://hermes.test/abc");
    const res = await GET(req, { params: { code: "abc" } });

    expect(res.status).toBe(301);
    const location = res.headers.get("location");
    expect(location).toMatch(/^https:\/\/example\.com\/?$/);
    expect(res.headers.get("Cache-Control")).toMatch(/max-age=31536000/);
    // If you ever want to assert calls:
    // expect((mockFetch as any).mock.calls.length).toBeGreaterThan(0);
  });
});
